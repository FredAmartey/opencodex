...
/**
 * Management write-boundary validation for `retryOnReset`, with the same fail-closed
 * contract as `retryOn429PolicyConfigError`: the load-time schema degrades a malformed block
 * to "absent", so this is the one place a bad value is refused instead of silently dropped.
 */
...
function strictPolicyConfigError(
  field: string,
  schema: { safeParse: (value: unknown) => { success: true } | { success: false; error: { issues: Array<{ code: string; message: string; path: PropertyKey[]; keys?: string[] }> } } },
  policy: unknown,
): string | null {
  ...
  const last = String(first.path[first.path.length - 1]);
  return `${field}.${last} is invalid (${first.message})`;
}
