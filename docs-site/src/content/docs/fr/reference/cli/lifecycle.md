...
Si des processus Codex `app-server` de longue durée sont encore actifs, `ocx sync` les classe avant de dire quoi que ce soit : un processus démarré après l’écriture détient déjà le catalogue sur disque et ne déclenche aucun avertissement, tandis qu’un processus démarré avant, ou dont l’heure de démarrage est illisible, avertit qu’il peut continuer à servir l’ancienne liste de modèles conservée en mémoire, même après la mise à jour de `opencodex-catalog.json` / `models_cache.json`. Ajoutez `--restart-codex` pour redémarrer les processus `codex … app-server` et `codex-code-mode-host` correspondants **et** quitter puis relancer entièrement l’application Codex Desktop, sous macOS, Linux et Windows, afin que le sélecteur de modèles relise le catalogue. Les conversations en cours se terminent. La recherche générale `pkill -f codex` est volontairement évitée.
